...
module ProposalsHelper
  ...
  def default_active_proposal_footer_tab?(tab)
    return true if tab == "comments" && projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.show_comments')

    return true if tab == "notifications" && projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.enable_proposal_notifications_tab') &&
      !projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.show_comments')

    tab == "milestones" && projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.enable_proposal_milestones_tab') &&
      !projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.show_comments') &&
      !projekt_feature?(@proposal.projekt_phase.projekt, 'proposals.enable_proposal_notifications_tab')
  end
end
