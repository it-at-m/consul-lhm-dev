define(['exports', './Matrix3-315394f6', './Check-666ab1a0', './defaultValue-0a909f67', './Math-2dbd6b93'], function (exports, Matrix3, Check, defaultValue, Math$1) {
  'use strict';
  ...
  var scratchCart1 = new Matrix3.Cartesian3();
  var scratchCart2 = new Matrix3.Cartesian3();
  function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
    var firstCartesian = Matrix3.Cartesian3.normalize(ellipsoid.cartographicToCartesian(start, scratchCart2), scratchCart1);
    var lastCartesian = Matrix3.Cartesian3.normalize(ellipsoid.cartographicToCartesian(end, scratchCart2), scratchCart2);

    //>>includeStart('debug', pragmas.debug);
    Check.Check.typeOf.number.greaterThanOrEquals("value", Math.abs(Math.abs(Matrix3.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI), 0.0125);
    //>>includeEnd('debug');
    ...
    ellipsoidRhumbLine._distance = calculateArcLength(ellipsoidRhumbLine, ellipsoid.maximumRadius, ellipsoid.minimumRadius, start.longitude, start.latitude, end.longitude, end.latitude);
  }
  ...
  function EllipsoidRhumbLine(start, end, ellipsoid) {
    var e = defaultValue.defaultValue(ellipsoid, Matrix3.Ellipsoid.WGS84);
    this._ellipsoid = e;
