define(['exports', './Transforms-40229881', './Matrix2-13178034', './Matrix3-315394f6', './Check-666ab1a0', './ComponentDatatype-f7b11d02', './defaultValue-0a909f67', './EllipseGeometryLibrary-2939e1dc', './GeometryAttribute-7d6f1732', './GeometryAttributes-f06a2792', './GeometryInstance-451dc1cd', './GeometryOffsetAttribute-04332ce7', './GeometryPipeline-ce4339ed', './IndexDatatype-a55ceaa1', './Math-2dbd6b93', './VertexFormat-6b480673'], function (exports, Transforms, Matrix2, Matrix3, Check, ComponentDatatype, defaultValue, EllipseGeometryLibrary, GeometryAttribute, GeometryAttributes, GeometryInstance, GeometryOffsetAttribute, GeometryPipeline, IndexDatatype, Math$1, VertexFormat) {
  'use strict';
  ...
  var projectedCenterScratch = new Matrix3.Cartesian3();
  ...
  function computeTopBottomAttributes(positions, options, extrude) {
    ...
    var minTexCoord = Matrix2.Cartesian2.fromElements(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, scratchMinTexCoord);
    var maxTexCoord = Matrix2.Cartesian2.fromElements(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, scratchMaxTexCoord);
    ...
    for (var i = 0; i < length; i += 3) {
      var i1 = i + 1;
      var i2 = i + 2;
      var position = Matrix3.Cartesian3.fromArray(positions, i, scratchCartesian1);
      if (vertexFormat.st) {
        var rotatedPoint = Matrix3.Matrix3.multiplyByVector(textureMatrix, position, scratchCartesian2);
  ...
  function computeWallAttributes(positions, options) {
    ...
    ellipsoid.geodeticSurfaceNormal(geodeticNormal, geodeticNormal);
    var rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, stRotation, quaternionScratch);
    var textureMatrix = Matrix3.Matrix3.fromQuaternion(rotation, textureMatrixScratch);
    var minTexCoord = Matrix2.Cartesian2.fromElements(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, scratchMinTexCoord);
    var maxTexCoord = Matrix2.Cartesian2.fromElements(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, scratchMaxTexCoord);
    var length = positions.length;
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      var scaledNormal = Matrix3.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4);
      ...
      if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
        ...
        var next = Matrix3.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4);
        Matrix3.Cartesian3.subtract(next, position, next);
        var bottom = Matrix3.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3);
        normal = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(bottom, next, normal), normal);
        if (vertexFormat.normal) {
          normals[i] = normal.x;
          normals[i1] = normal.y;
          normals[i2] = normal.z;
  ...
  function computeExtrudedEllipse(options) {
    ...
    var geo = GeometryPipeline.GeometryPipeline.combineInstances([new GeometryInstance.GeometryInstance({
   ...
   * @type {Number}
   */
  EllipseGeometry.packedLength = Matrix3.Cartesian3.packedLength + Matrix3.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + 9;
