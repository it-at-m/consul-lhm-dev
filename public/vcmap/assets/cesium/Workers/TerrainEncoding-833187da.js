define(['exports', './Transforms-40229881', './Matrix3-315394f6', './Check-666ab1a0', './defaultValue-0a909f67', './Matrix2-13178034', './AttributeCompression-b646d393', './ComponentDatatype-f7b11d02', './Math-2dbd6b93'], function (exports, Transforms, Matrix3, Check, defaultValue, Matrix2, AttributeCompression, ComponentDatatype, Math$1) {
  'use strict';
  ...
  var scratchCameraPositionInScaledSpaceShrunk = new Matrix3.Cartesian3();
  ...
  var scratchEllipsoidShrunk = Matrix3.Ellipsoid.clone(Matrix3.Ellipsoid.UNIT_SPHERE);
  ...
  EllipsoidalOccluder.prototype.computeHorizonCullingPointFromRectangle = function (rectangle, ellipsoid, result) {
    ...
    var positions = Matrix2.Rectangle.subsample(rectangle, ellipsoid, 0.0, subsampleScratch);
  ...
  var scratchEllipsoidShrunkRadii = new Matrix3.Cartesian3();
  function getPossiblyShrunkEllipsoid(ellipsoid, minimumHeight, result) {
    if (defaultValue.defined(minimumHeight) && minimumHeight < 0.0 && ellipsoid.minimumRadius > -minimumHeight) {
      var ellipsoidShrunkRadii = Matrix3.Cartesian3.fromElements(ellipsoid.radii.x + minimumHeight, ellipsoid.radii.y + minimumHeight, ellipsoid.radii.z + minimumHeight, scratchEllipsoidShrunkRadii);
  ...
  var positionScratch = new Matrix3.Cartesian3();
  ...
  function isScaledSpacePointVisible(occludeeScaledSpacePosition, cameraPositionInScaledSpace, distanceToLimbInScaledSpaceSquared) {
    ...
    var vt = Matrix3.Cartesian3.subtract(occludeeScaledSpacePosition, cv, scratchCartesian);
    var vtDotVc = -Matrix3.Cartesian3.dot(vt, cv);
    // If vhMagnitudeSquared < 0 then we are below the surface of the ellipsoid and
    // in this case, set the culling plane to be on V.
    var isOccluded = vhMagnitudeSquared < 0 ? vtDotVc > 0 : vtDotVc > vhMagnitudeSquared && vtDotVc * vtDotVc / Matrix3.Cartesian3.magnitudeSquared(vt) > vhMagnitudeSquared;
    return !isOccluded;
  }
  var scaledSpaceScratch = new Matrix3.Cartesian3();
  var directionScratch = new Matrix3.Cartesian3();
  function computeMagnitude(ellipsoid, position, scaledSpaceDirectionToPoint) {
    var scaledSpacePosition = ellipsoid.transformPositionToScaledSpace(position, scaledSpaceScratch);
    var magnitudeSquared = Matrix3.Cartesian3.magnitudeSquared(scaledSpacePosition);
    var magnitude = Math.sqrt(magnitudeSquared);
    var direction = Matrix3.Cartesian3.divideByScalar(scaledSpacePosition, magnitude, directionScratch);

    // For the purpose of this computation, points below the ellipsoid are consider to be on it instead.
    magnitudeSquared = Math.max(1.0, magnitudeSquared);
    magnitude = Math.max(1.0, magnitude);
    var cosAlpha = Matrix3.Cartesian3.dot(direction, scaledSpaceDirectionToPoint);
    var sinAlpha = Matrix3.Cartesian3.magnitude(Matrix3.Cartesian3.cross(direction, scaledSpaceDirectionToPoint, direction));
  ...
  var directionToPointScratch = new Matrix3.Cartesian3();
  ...
  var scratchCartographic = new Matrix3.Cartesian3();
  ...
  TerrainExaggeration.getPosition = function (position, ellipsoid, terrainExaggeration, terrainExaggerationRelativeHeight, result) {
    ...
    return Matrix3.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, newHeight, ellipsoid, result);
  };
  ...
  var cartesian3DimScratch = new Matrix3.Cartesian3();
  ...
  function TerrainEncoding(center, axisAlignedBoundingBox, minimumHeight, maximumHeight, fromENU, hasVertexNormals, hasWebMercatorT, hasGeodeticSurfaceNormals, exaggeration, exaggerationRelativeHeight) {
    ...
    if (defaultValue.defined(axisAlignedBoundingBox) && defaultValue.defined(minimumHeight) && defaultValue.defined(maximumHeight) && defaultValue.defined(fromENU)) {
      ...
      var dimensions = Matrix3.Cartesian3.subtract(maximum, minimum, cartesian3DimScratch);
      var hDim = maximumHeight - minimumHeight;
      var maxDim = Math.max(Matrix3.Cartesian3.maximumComponent(dimensions), hDim);
      ...
      var translation = Matrix3.Cartesian3.negate(minimum, cartesian3Scratch);
      Matrix2.Matrix4.multiply(Matrix2.Matrix4.fromTranslation(translation, matrix4Scratch), toENU, toENU);
  ...
  var scratchGeodeticSurfaceNormal = new Matrix3.Cartesian3();
  ...
  TerrainEncoding.prototype.getExaggeratedPosition = function (buffer, index, result) {
    ...
    if (hasExaggeration && this.hasGeodeticSurfaceNormals) {
      ...
      var heightDifference = TerrainExaggeration$1.getHeight(rawHeight, exaggeration, exaggerationRelativeHeight) - rawHeight;
