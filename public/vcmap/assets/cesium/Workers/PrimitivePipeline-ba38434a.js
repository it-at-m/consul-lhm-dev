define(['exports', './Transforms-40229881', './ComponentDatatype-f7b11d02', './defaultValue-0a909f67', './Check-666ab1a0', './Matrix3-315394f6', './GeometryAttribute-7d6f1732', './GeometryAttributes-f06a2792', './GeometryPipeline-ce4339ed', './IndexDatatype-a55ceaa1', './Matrix2-13178034', './WebMercatorProjection-13a90d41'], function (exports, Transforms, ComponentDatatype, defaultValue, Check, Matrix3, GeometryAttribute, GeometryAttributes, GeometryPipeline, IndexDatatype, Matrix2, WebMercatorProjection) {
  ...
  function transformToWorldCoordinates(instances, primitiveModelMatrix, scene3DOnly) {
    ...
    if (toWorld) {
      ...
      Matrix2.Matrix4.multiplyTransformation(primitiveModelMatrix, instances[0].modelMatrix, primitiveModelMatrix);
    }
  }
      ...
      if (!scene3DOnly) {
        for (var name in attributes) {
          if (attributes.hasOwnProperty(name) && attributes[name].componentDatatype === ComponentDatatype.ComponentDatatype.DOUBLE) {
            var name3D = "".concat(name, "3D");
            var name2D = "".concat(name, "2D");
            ...
            GeometryPipeline.GeometryPipeline.encodeAttribute(geometry, name3D, "".concat(name3D, "High"), "".concat(name3D, "Low"));
            GeometryPipeline.GeometryPipeline.encodeAttribute(geometry, name2D, "".concat(name2D, "High"), "".concat(name2D, "Low"));
          }
        }
      } else {
        for (var _name in attributes) {
          if (attributes.hasOwnProperty(_name) && attributes[_name].componentDatatype === ComponentDatatype.ComponentDatatype.DOUBLE) {
            GeometryPipeline.GeometryPipeline.encodeAttribute(geometry, _name, "".concat(_name, "3DHigh"), "".concat(_name, "3DLow"));
          }
        }
      }
  ...
  function countCreateGeometryResults(items) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      var attributes = geometry.attributes;
      count += 7 + 2 * Transforms.BoundingSphere.packedLength + (defaultValue.defined(geometry.indices) ? geometry.indices.length : 0);
  ...
  PrimitivePipeline.unpackCreateGeometryResults = function (createGeometryResult) {
    ...
    while (packedGeometryIndex < packedGeometry.length) {
      ...
      var length = void 0;
      var values = void 0;
      var componentsPerAttribute = void 0;
  ...
  PrimitivePipeline.unpackCombineGeometryParameters = function (packedParameters) {
    ...
    var ellipsoid = Matrix3.Ellipsoid.clone(packedParameters.ellipsoid);
