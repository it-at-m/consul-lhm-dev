define(['./defaultValue-0a909f67'], function (defaultValue) {
  ...
  function formatError(object) {
    ...
    if (defaultValue.defined(name) && defaultValue.defined(message)) {
      result = "".concat(name, ": ").concat(message);
    } else {
  ...
  function createTaskProcessorWorker(workerFunction) {
    ...
    return function (event) {
      ...
      return Promise.resolve(callAndWrap(workerFunction, data.parameters, transferableObjects)).then(function (result) {
        ...
        try {
          ...
          responseMessage.error = "postMessage failed with error: ".concat(formatError(e), "\n  with responseMessage: ").concat(JSON.stringify(responseMessage));
