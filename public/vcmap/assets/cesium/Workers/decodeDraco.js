define(['./ComponentDatatype-f7b11d02', './defaultValue-0a909f67', './IndexDatatype-a55ceaa1', './RuntimeError-06c93819', './createTaskProcessorWorker', './Check-666ab1a0', './WebGLConstants-a8cc3e8c', './Math-2dbd6b93'], function (ComponentDatatype, defaultValue, IndexDatatype, RuntimeError, createTaskProcessorWorker, Check, WebGLConstants, Math) {
  'use strict';
  ...
  function decodePointCloud(parameters) {
    ...
    if (!decodingStatus.ok() || dracoPointCloud.ptr === 0) {
      throw new RuntimeError.RuntimeError("Error decoding draco point cloud: ".concat(decodingStatus.error_msg()));
    }
    ...
    for (var propertyName in properties) {
      if (properties.hasOwnProperty(propertyName)) {
        var dracoAttribute = void 0;
        if (propertyName === "POSITION" || propertyName === "NORMAL") {
          var dracoAttributeId = dracoDecoder.GetAttributeId(dracoPointCloud, draco[propertyName]);
  ...
  function decodePrimitive(parameters) {
    ...
    if (!decodingStatus.ok() || dracoGeometry.ptr === 0) {
      throw new RuntimeError.RuntimeError("Error decoding draco mesh geometry: ".concat(decodingStatus.error_msg()));
    }
    draco.destroy(buffer);
    var attributeData = {};
