define(['./defaultValue-0a909f67', './Matrix3-315394f6', './ArcType-ce2e50ab', './BoundingRectangle-be5924f4', './Transforms-40229881', './Matrix2-13178034', './Check-666ab1a0', './ComponentDatatype-f7b11d02', './EllipsoidGeodesic-98c62a56', './EllipsoidTangentPlane-214683dc', './GeometryAttribute-7d6f1732', './GeometryInstance-451dc1cd', './GeometryOffsetAttribute-04332ce7', './GeometryPipeline-ce4339ed', './IndexDatatype-a55ceaa1', './Math-2dbd6b93', './PolygonGeometryLibrary-a8680d96', './PolygonPipeline-844aab0a', './VertexFormat-6b480673', './combine-ca22a614', './RuntimeError-06c93819', './WebGLConstants-a8cc3e8c', './AxisAlignedBoundingBox-ff186ccc', './IntersectionTests-f6e6bd8a', './Plane-900aa728', './AttributeCompression-b646d393', './EncodedCartesian3-81f70735', './arrayRemoveDuplicates-c2038105', './EllipsoidRhumbLine-19756602', './GeometryAttributes-f06a2792'], function (defaultValue, Matrix3, ArcType, BoundingRectangle, Transforms, Matrix2, Check, ComponentDatatype, EllipsoidGeodesic, EllipsoidTangentPlane, GeometryAttribute, GeometryInstance, GeometryOffsetAttribute, GeometryPipeline, IndexDatatype, Math$1, PolygonGeometryLibrary, PolygonPipeline, VertexFormat, combine, RuntimeError, WebGLConstants, AxisAlignedBoundingBox, IntersectionTests, Plane, AttributeCompression, EncodedCartesian3, arrayRemoveDuplicates, EllipsoidRhumbLine, GeometryAttributes) {
  ...
  var scratchPerPosNormal = new Matrix3.Cartesian3();
  var scratchPerPosTangent = new Matrix3.Cartesian3();
  var scratchPerPosBitangent = new Matrix3.Cartesian3();
  var appendTextureCoordinatesOrigin = new Matrix2.Cartesian2();
  var appendTextureCoordinatesCartesian2 = new Matrix2.Cartesian2();
  var appendTextureCoordinatesCartesian3 = new Matrix3.Cartesian3();
  var appendTextureCoordinatesQuaternion = new Transforms.Quaternion();
  var appendTextureCoordinatesMatrix3 = new Matrix3.Matrix3();
  var tangentMatrixScratch = new Matrix3.Matrix3();
  function computeAttributes(options) {
    var vertexFormat = options.vertexFormat;
    var geometry = options.geometry;
    var shadowVolume = options.shadowVolume;
    var flatPositions = geometry.attributes.position.values;
    var flatTexcoords = defaultValue.defined(geometry.attributes.st) ? geometry.attributes.st.values : undefined;
    ...
    if (vertexFormat.st || vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
      ...
      if (stRotation !== 0.0) {
        ...
        tangentRotationMatrix = Matrix3.Matrix3.clone(Matrix3.Matrix3.IDENTITY, tangentRotationMatrix);
      }
      var bottomOffset = 0;
      var bottomOffset2 = 0;
      if (top && bottom) {
      ...
      for (var i = 0; i < length; i += 3) {
        var position = Matrix3.Cartesian3.fromArray(flatPositions, i, appendTextureCoordinatesCartesian3);
        if (vertexFormat.st) {
          if (!defaultValue.defined(flatTexcoords)) {
            ...
            var stx = Math$1.CesiumMath.clamp(st.x / boundingRectangle.width, 0, 1);
            var sty = Math$1.CesiumMath.clamp(st.y / boundingRectangle.height, 0, 1);
            if (bottom) {
        ...
        if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
          ...
          if (wall) {
            if (i + 3 < length) {
              var p1 = Matrix3.Cartesian3.fromArray(flatPositions, i + 3, p1Scratch);
              if (recomputeNormal) {
                var p2 = Matrix3.Cartesian3.fromArray(flatPositions, i + length, p2Scratch);
            ...
            if (vertexFormat.tangent || vertexFormat.bitangent) {
              if (perPositionHeight) {
                scratchPerPosNormal = Matrix3.Cartesian3.fromArray(normals, attrIndex, scratchPerPosNormal);
                scratchPerPosTangent = Matrix3.Cartesian3.cross(Matrix3.Cartesian3.UNIT_Z, scratchPerPosNormal, scratchPerPosTangent);
                scratchPerPosTangent = Matrix3.Cartesian3.normalize(Matrix3.Matrix3.multiplyByVector(tangentRotationMatrix, scratchPerPosTangent, scratchPerPosTangent), scratchPerPosTangent);
                if (vertexFormat.bitangent) {
                  scratchPerPosBitangent = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(scratchPerPosNormal, scratchPerPosTangent, scratchPerPosBitangent), scratchPerPosBitangent);
                }
              }
              tangent = Matrix3.Cartesian3.cross(Matrix3.Cartesian3.UNIT_Z, normal, tangent);
              tangent = Matrix3.Cartesian3.normalize(Matrix3.Matrix3.multiplyByVector(tangentRotationMatrix, tangent, tangent), tangent);
              if (vertexFormat.bitangent) {
  ...
  function computeRectangle(positions, ellipsoid, arcType, granularity, result) {
    ...
    var inverseChordLength = 1.0 / Math$1.CesiumMath.chordLength(granularity, ellipsoid.maximumRadius);
  ...
  function interpolateAndGrowRectangle(ellipsoidGeodesic, inverseChordLength, result, idlCross) {
    ...
    for (var i = 0; i < numPoints; i++) {
      ...
      var lonAdjusted = longitude >= 0 ? longitude : longitude + Math$1.CesiumMath.TWO_PI;
  ...
  function createGeometryFromPositionsExtruded(ellipsoid, polygon, textureCoordinates, granularity, hierarchy, perPositionHeight, closeTop, closeBottom, vertexFormat, arcType) {
    ...
    if (closeTop || closeBottom) {
      var topGeo = PolygonGeometryLibrary.PolygonGeometryLibrary.createGeometryFromPositions(ellipsoid, polygon, textureCoordinates, granularity, perPositionHeight, vertexFormat, arcType);
      ...
      if (closeTop && closeBottom) {
        ...
        if (vertexFormat.st && defaultValue.defined(textureCoordinates)) {
          var texcoords = topGeo.attributes.st.values;
    ...
    var wallGeo = PolygonGeometryLibrary.PolygonGeometryLibrary.computeWallGeometry(outerRing, textureCoordinates, ellipsoid, granularity, perPositionHeight, arcType);
    ...
    for (i = 0; i < holes.length; i++) {
      ...
      wallGeo = PolygonGeometryLibrary.PolygonGeometryLibrary.computeWallGeometry(hole, textureCoordinates, ellipsoid, granularity, perPositionHeight, arcType);
      geos.walls.push(new GeometryInstance.GeometryInstance({
        geometry: wallGeo
      }));
    }
  ...
  function PolygonGeometry(options) {
     ...
     * @type {Number}
     */
    this.packedLength = PolygonGeometryLibrary.PolygonGeometryLibrary.computeHierarchyPackedLength(polygonHierarchy, Matrix3.Cartesian3) + Matrix3.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + (textureCoordinates ? PolygonGeometryLibrary.PolygonGeometryLibrary.computeHierarchyPackedLength(textureCoordinates, Matrix2.Cartesian2) : 1) + 12;
  }
  ...
  PolygonGeometry.createGeometry = function (polygonGeometry) {
    ...
    var results = PolygonGeometryLibrary.PolygonGeometryLibrary.polygonsFromHierarchy(polygonHierarchy, hasTextureCoordinates, tangentPlane.projectPointsOntoPlane.bind(tangentPlane), !perPositionHeight, ellipsoid);
    ...
    var extrude = polygonGeometry._perPositionHeightExtrude || !Math$1.CesiumMath.equalsEpsilon(height, extrudedHeight, 0, Math$1.CesiumMath.EPSILON2);
    var options = {
    ...
    if (extrude) {
      ...
      for (i = 0; i < polygons.length; i++) {
        var splitGeometry = createGeometryFromPositionsExtruded(ellipsoid, polygons[i], hasTextureCoordinates ? textureCoordinatePolygons[i] : undefined, granularity, hierarchy[i], perPositionHeight, closeTop, closeBottom, vertexFormat, arcType);
        var topAndBottom = void 0;
        if (closeTop && closeBottom) {
      ...
      for (i = 0; i < polygons.length; i++) {
        var geometryInstance = new GeometryInstance.GeometryInstance({
          geometry: PolygonGeometryLibrary.PolygonGeometryLibrary.createGeometryFromPositions(ellipsoid, polygons[i], hasTextureCoordinates ? textureCoordinatePolygons[i] : undefined, granularity, perPositionHeight, vertexFormat, arcType)
        });
