define(['./AttributeCompression-b646d393', './Matrix3-315394f6', './combine-ca22a614', './IndexDatatype-a55ceaa1', './Math-2dbd6b93', './Matrix2-13178034', './createTaskProcessorWorker', './ComponentDatatype-f7b11d02', './defaultValue-0a909f67', './Check-666ab1a0', './WebGLConstants-a8cc3e8c', './RuntimeError-06c93819'], function (AttributeCompression, Matrix3, combine, IndexDatatype, Math$1, Matrix2, createTaskProcessorWorker, ComponentDatatype, defaultValue, Check, WebGLConstants, RuntimeError) {
  ...
  var MAX_SHORT = 32767;
  var MITER_BREAK = Math.cos(Math$1.CesiumMath.toRadians(150.0));
  ...
  function decodePositions(uBuffer, vBuffer, heightBuffer, rectangle, minimumHeight, maximumHeight, ellipsoid) {
    ...
    for (var i = 0; i < positionsLength; ++i) {
      ...
      var lon = Math$1.CesiumMath.lerp(rectangle.west, rectangle.east, u / MAX_SHORT);
      var lat = Math$1.CesiumMath.lerp(rectangle.south, rectangle.north, v / MAX_SHORT);
      var alt = Math$1.CesiumMath.lerp(minimumHeight, maximumHeight, h / MAX_SHORT);
  ...
  var previousCompressedCartographicScratch = new Matrix3.Cartographic();
  var currentCompressedCartographicScratch = new Matrix3.Cartographic();
  ...
  var towardCurrScratch = new Matrix3.Cartesian3();
  var towardNextScratch = new Matrix3.Cartesian3();
  function computeMiteredNormal(previousPosition, position, nextPosition, ellipsoidSurfaceNormal, result) {
    var towardNext = Matrix3.Cartesian3.subtract(nextPosition, position, towardNextScratch);
    var towardCurr = Matrix3.Cartesian3.subtract(position, previousPosition, towardCurrScratch);
  ...
  //       0-----------4
  //
  var REFERENCE_INDICES = [0, 2, 6, 0, 6, 4,
  // right
  0, 1, 3, 0, 3, 2,
  // start face
  0, 4, 5, 0, 5, 1,
  ...
  var scratchStartEllipsoidNormal = new Matrix3.Cartesian3();
  var scratchStartFaceNormal = new Matrix3.Cartesian3();
  var scratchEndEllipsoidNormal = new Matrix3.Cartesian3();
  var scratchEndFaceNormal = new Matrix3.Cartesian3();
  VertexAttributesAndIndices.prototype.addVolume = function (preStartRTC, startRTC, endRTC, postEndRTC, startHeight, endHeight, halfWidth, batchId, center, ellipsoid) {
    var position = Matrix3.Cartesian3.add(startRTC, center, positionScratch);
  ...
  function createVectorTileClampedPolylines(parameters, transferableObjects) {
    ...
    for (i = 0; i < countsLength; i++) {
      ...
      for (var j = 0; j < polylineVolumeCount; j++) {
        var volumeStart = Matrix3.Cartesian3.unpack(positionsRTC, currentPositionIndex, scratchP0);
        var volumeEnd = Matrix3.Cartesian3.unpack(positionsRTC, currentPositionIndex + 3, scratchP1);
        ...
        if (j === 0) {
          ...
          var finalPosition = Matrix3.Cartesian3.unpack(positionsRTC, finalPositionIndex, scratchPrev);
          if (Matrix3.Cartesian3.equals(finalPosition, volumeStart)) {
            ...
            var offsetPastStart = Matrix3.Cartesian3.subtract(volumeStart, volumeEnd, scratchPrev);
        ...
        if (j === polylineVolumeCount - 1) {
          // Check if this volume is like a loop
          var firstPosition = Matrix3.Cartesian3.unpack(positionsRTC, volumeFirstPositionIndex, scratchNext);
          if (Matrix3.Cartesian3.equals(firstPosition, volumeEnd)) {
            ...
            var offsetPastEnd = Matrix3.Cartesian3.subtract(volumeEnd, volumeStart, scratchNext);
