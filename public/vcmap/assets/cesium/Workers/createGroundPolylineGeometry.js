define(['./Transforms-40229881', './Matrix2-13178034', './Matrix3-315394f6', './Check-666ab1a0', './defaultValue-0a909f67', './Math-2dbd6b93', './ArcType-ce2e50ab', './arrayRemoveDuplicates-c2038105', './ComponentDatatype-f7b11d02', './EllipsoidGeodesic-98c62a56', './EllipsoidRhumbLine-19756602', './EncodedCartesian3-81f70735', './GeometryAttribute-7d6f1732', './IntersectionTests-f6e6bd8a', './Plane-900aa728', './WebMercatorProjection-13a90d41', './combine-ca22a614', './RuntimeError-06c93819', './WebGLConstants-a8cc3e8c'], function (Transforms, Matrix2, Matrix3, Check, defaultValue, Math$1, ArcType, arrayRemoveDuplicates, ComponentDatatype, EllipsoidGeodesic, EllipsoidRhumbLine, EncodedCartesian3, GeometryAttribute, IntersectionTests, Plane, WebMercatorProjection, combine, RuntimeError, WebGLConstants) {
  ...
  GeographicTilingScheme.prototype.rectangleToNativeRectangle = function (rectangle, result) {
    ...
    var west = Math$1.CesiumMath.toDegrees(rectangle.west);
    var south = Math$1.CesiumMath.toDegrees(rectangle.south);
    var east = Math$1.CesiumMath.toDegrees(rectangle.east);
    var north = Math$1.CesiumMath.toDegrees(rectangle.north);
  ...
  var scratchDiagonalCartesianNE = new Matrix3.Cartesian3();
  var scratchDiagonalCartesianSW = new Matrix3.Cartesian3();
  var scratchDiagonalCartographic = new Matrix3.Cartographic();
  var scratchCenterCartesian = new Matrix3.Cartesian3();
  var scratchSurfaceCartesian = new Matrix3.Cartesian3();
  var scratchBoundingSphere = new Transforms.BoundingSphere();
  var tilingScheme = new GeographicTilingScheme();
  var scratchCorners = [new Matrix3.Cartographic(), new Matrix3.Cartographic(), new Matrix3.Cartographic(), new Matrix3.Cartographic()];
  ...
  ApproximateTerrainHeights.initialize = function () {
    ...
    initPromise = Transforms.Resource.fetchJson(Transforms.buildModuleUrl("Assets/approximateTerrainHeights.json")).then(function (json) {
  ...
  ApproximateTerrainHeights.getMinimumMaximumHeights = function (rectangle, ellipsoid) {
    ...
    if (defaultValue.defined(xyLevel)) {
      var key = "".concat(xyLevel.level, "-").concat(xyLevel.x, "-").concat(xyLevel.y);
      ...
      ellipsoid.cartographicToCartesian(Matrix2.Rectangle.northeast(rectangle, scratchDiagonalCartographic), scratchDiagonalCartesianNE);
      ellipsoid.cartographicToCartesian(Matrix2.Rectangle.southwest(rectangle, scratchDiagonalCartographic), scratchDiagonalCartesianSW);
      Matrix3.Cartesian3.midpoint(scratchDiagonalCartesianSW, scratchDiagonalCartesianNE, scratchCenterCartesian);
      ...
      if (defaultValue.defined(surfacePosition)) {
        var distance = Matrix3.Cartesian3.distance(scratchCenterCartesian, surfacePosition);
  ...
  var MITER_BREAK_SMALL = Math.cos(Math$1.CesiumMath.toRadians(30.0));
  var MITER_BREAK_LARGE = Math.cos(Math$1.CesiumMath.toRadians(150.0));
  ...
  Object.defineProperties(GroundPolylineGeometry.prototype, {
    ...
    packedLength: {
      get: function get() {
        return 1.0 + this._positions.length * 3 + 1.0 + 1.0 + 1.0 + Matrix3.Ellipsoid.packedLength + 1.0 + 1.0;
      }
    }
  });
  ...
  var cart3Scratch1 = new Matrix3.Cartesian3();
  var cart3Scratch2 = new Matrix3.Cartesian3();
  var cart3Scratch3 = new Matrix3.Cartesian3();
  ...
  var interpolatedCartographicScratch = new Matrix3.Cartographic();
  var interpolatedBottomScratch = new Matrix3.Cartesian3();
  var interpolatedTopScratch = new Matrix3.Cartesian3();
  var interpolatedNormalScratch = new Matrix3.Cartesian3();
  ...
  var heightlessCartographicScratch = new Matrix3.Cartographic();
  ...
  GroundPolylineGeometry.unpack = function (array, startingIndex, result) {
    ...
    var ellipsoid = Matrix3.Ellipsoid.unpack(array, index);
    index += Matrix3.Ellipsoid.packedLength;
  ...
  var toPreviousScratch = new Matrix3.Cartesian3();
  var toNextScratch = new Matrix3.Cartesian3();
  var forwardScratch = new Matrix3.Cartesian3();
  var vertexUpScratch = new Matrix3.Cartesian3();
  var cosine90 = 0.0;
  var cosine180 = -1.0;
  function computeVertexMiterNormal(previousBottom, vertexBottom, vertexTop, nextBottom, result) {
    ...
    if (Math$1.CesiumMath.equalsEpsilon(Matrix3.Cartesian3.dot(toPrevious, toNext), cosine180, Math$1.CesiumMath.EPSILON5)) {
    ...
    var forward = Matrix3.Cartesian3.cross(up, result, forwardScratch);
    if (Matrix3.Cartesian3.dot(toNext, forward) < cosine90) {
      result = Matrix3.Cartesian3.negate(result, result);
    }
    return result;
  }
  var XZ_PLANE = Plane.Plane.fromPointNormal(Matrix3.Cartesian3.ZERO, Matrix3.Cartesian3.UNIT_Y);
  var previousBottomScratch = new Matrix3.Cartesian3();
  var vertexBottomScratch = new Matrix3.Cartesian3();
  var vertexTopScratch = new Matrix3.Cartesian3();
  var nextBottomScratch = new Matrix3.Cartesian3();
  var vertexNormalScratch = new Matrix3.Cartesian3();
  var intersectionScratch = new Matrix3.Cartesian3();
  var cartographicScratch0 = new Matrix3.Cartographic();
  var cartographicScratch1 = new Matrix3.Cartographic();
  var cartographicIntersectionScratch = new Matrix3.Cartographic();
  /**
  ...
  GroundPolylineGeometry.createGeometry = function (groundPolylineGeometry) {
    ...
    for (i = 0; i < positionsLength - 1; i++) {
      ...
      if (defaultValue.defined(intersection) && !Matrix3.Cartesian3.equalsEpsilon(intersection, p0, Math$1.CesiumMath.EPSILON7) && !Matrix3.Cartesian3.equalsEpsilon(intersection, p1, Math$1.CesiumMath.EPSILON7)) {
    ...
    for (i = 0; i < cartographicsLength; i++) {
      var cartographic = Matrix3.Cartographic.fromCartesian(splitPositions[i], ellipsoid);
      cartographic.height = 0.0;
      cartographics[i] = cartographic;
    }
    cartographics = arrayRemoveDuplicates.arrayRemoveDuplicates(cartographics, Matrix3.Cartographic.equalsEpsilon);
  ...
  var lineDirectionScratch = new Matrix3.Cartesian3();
  var matrix3Scratch = new Matrix3.Matrix3();
  ...
  function breakMiter(endGeometryNormal, startBottom, endBottom, endTop) {
    ...
    var dot = Matrix3.Cartesian3.dot(lineDirection, endGeometryNormal);
    if (dot > MITER_BREAK_SMALL || dot < MITER_BREAK_LARGE) {
      var vertexUp = direction(endTop, endBottom, vertexUpScratch);
      var angle = dot < MITER_BREAK_LARGE ? Math$1.CesiumMath.PI_OVER_TWO : -Math$1.CesiumMath.PI_OVER_TWO;
      var quaternion = Transforms.Quaternion.fromAxisAngle(vertexUp, angle, quaternionScratch);
      var rotationMatrix = Matrix3.Matrix3.fromQuaternion(quaternion, matrix3Scratch);
      Matrix3.Matrix3.multiplyByVector(rotationMatrix, endGeometryNormal, endGeometryNormal);
      return true;
    }
    return false;
  }
  var endPosCartographicScratch = new Matrix3.Cartographic();
  var normalStartpointScratch = new Matrix3.Cartesian3();
  var normalEndpointScratch = new Matrix3.Cartesian3();
  function projectNormal(projection, cartographic, normal, projectedPosition, result) {
    var position = Matrix3.Cartographic.toCartesian(cartographic, projection._ellipsoid, normalStartpointScratch);
    var normalEndpoint = Matrix3.Cartesian3.add(position, normal, normalEndpointScratch);
    ...
    if (Math.abs(cartographic.longitude - normalEndpointCartographic.longitude) > Math$1.CesiumMath.PI_OVER_TWO) {
      flipNormal = true;
  ...
  var adjustHeightNormalScratch = new Matrix3.Cartesian3();
  var adjustHeightOffsetScratch = new Matrix3.Cartesian3();
  function adjustHeights(bottom, top, minHeight, maxHeight, adjustHeightBottom, adjustHeightTop) {
    // bottom and top should be at WALL_INITIAL_MIN_HEIGHT and WALL_INITIAL_MAX_HEIGHT, respectively
    var adjustHeightNormal = Matrix3.Cartesian3.subtract(top, bottom, adjustHeightNormalScratch);
    ...
    var adjustHeightOffset = Matrix3.Cartesian3.multiplyByScalar(adjustHeightNormal, distanceForBottom, adjustHeightOffsetScratch);
    Matrix3.Cartesian3.add(bottom, adjustHeightOffset, adjustHeightBottom);
    var distanceForTop = maxHeight - WALL_INITIAL_MAX_HEIGHT;
    adjustHeightOffset = Matrix3.Cartesian3.multiplyByScalar(adjustHeightNormal, distanceForTop, adjustHeightOffsetScratch);
    Matrix3.Cartesian3.add(top, adjustHeightOffset, adjustHeightTop);
  }
  var nudgeDirectionScratch = new Matrix3.Cartesian3();
  ...
  function nudgeCartographic(start, end) {
    ...
    if (Math$1.CesiumMath.equalsEpsilon(absStartLon, Math$1.CesiumMath.PI, Math$1.CesiumMath.EPSILON11)) {
      var endSign = Math$1.CesiumMath.sign(end.longitude);
      start.longitude = endSign * (absStartLon - Math$1.CesiumMath.EPSILON11);
      return 1;
    } else if (Math$1.CesiumMath.equalsEpsilon(absEndLon, Math$1.CesiumMath.PI, Math$1.CesiumMath.EPSILON11)) {
      var startSign = Math$1.CesiumMath.sign(start.longitude);
      end.longitude = startSign * (absEndLon - Math$1.CesiumMath.EPSILON11);
      return 2;
    }
    return 0;
  }
  var startCartographicScratch = new Matrix3.Cartographic();
  var endCartographicScratch = new Matrix3.Cartographic();
  var segmentStartTopScratch = new Matrix3.Cartesian3();
  var segmentEndTopScratch = new Matrix3.Cartesian3();
  var segmentStartBottomScratch = new Matrix3.Cartesian3();
  var segmentEndBottomScratch = new Matrix3.Cartesian3();
  var segmentStartNormalScratch = new Matrix3.Cartesian3();
  var segmentEndNormalScratch = new Matrix3.Cartesian3();
  ...
  var adjustHeightStartTopScratch = new Matrix3.Cartesian3();
  var adjustHeightEndTopScratch = new Matrix3.Cartesian3();
  var adjustHeightStartBottomScratch = new Matrix3.Cartesian3();
  var adjustHeightEndBottomScratch = new Matrix3.Cartesian3();
  var segmentStart2DScratch = new Matrix3.Cartesian3();
  var segmentEnd2DScratch = new Matrix3.Cartesian3();
  var segmentStartNormal2DScratch = new Matrix3.Cartesian3();
  var segmentEndNormal2DScratch = new Matrix3.Cartesian3();
  var offsetScratch = new Matrix3.Cartesian3();
  var startUpScratch = new Matrix3.Cartesian3();
  var endUpScratch = new Matrix3.Cartesian3();
  var rightScratch = new Matrix3.Cartesian3();
  var startPlaneNormalScratch = new Matrix3.Cartesian3();
  var endPlaneNormalScratch = new Matrix3.Cartesian3();
  var encodeScratch = new EncodedCartesian3.EncodedCartesian3();
  var encodeScratch2D = new EncodedCartesian3.EncodedCartesian3();
  var forwardOffset2DScratch = new Matrix3.Cartesian3();
  var right2DScratch = new Matrix3.Cartesian3();
  var normalNudgeScratch = new Matrix3.Cartesian3();
  var scratchBoundingSpheres = [new Transforms.BoundingSphere(), new Transforms.BoundingSphere()];

  // Winding order is reversed so each segment's volume is inside-out
  var REFERENCE_INDICES = [0, 2, 1, 0, 3, 2,
  // right
  0, 7, 3, 0, 4, 7,
  // start
  0, 5, 4, 0, 1, 5,
  ...
  function generateGeometryAttributes(loop, projection, bottomPositionsArray, topPositionsArray, normalsArray, cartographicsArray, compute2dAttributes) {
    ...
    var endBottom = Matrix3.Cartesian3.unpack(bottomPositionsArray, 0, segmentEndBottomScratch);
    var endTop = Matrix3.Cartesian3.unpack(topPositionsArray, 0, segmentEndTopScratch);
    var endGeometryNormal = Matrix3.Cartesian3.unpack(normalsArray, 0, segmentEndNormalScratch);
    if (loop) {
      var preEndBottom = Matrix3.Cartesian3.unpack(bottomPositionsArray, bottomPositionsArray.length - 6, segmentStartBottomScratch);
    ...
    for (i = 0; i < segmentCount; i++) {
      var startBottom = Matrix3.Cartesian3.clone(endBottom, segmentStartBottomScratch);
      var startTop = Matrix3.Cartesian3.clone(endTop, segmentStartTopScratch);
      var startGeometryNormal = Matrix3.Cartesian3.clone(endGeometryNormal, segmentStartNormalScratch);
      if (miterBroken) {
      ...
      var start2D = void 0;
      var end2D = void 0;
      var startGeometryNormal2D = void 0;
      var endGeometryNormal2D = void 0;
      if (compute2dAttributes) {
        ...
        if (nudgeResult === 0 || Matrix3.Cartesian3.dot(direction2D, Matrix3.Cartesian3.UNIT_Y) > MITER_BREAK_SMALL) {
          ...
          startGeometryNormal2D.y = Math$1.CesiumMath.sign(startCartographic.longitude - Math.abs(endCartographic.longitude));
          ...
          endGeometryNormal2D.y = Math$1.CesiumMath.sign(startCartographic.longitude - endCartographic.longitude);
      ...
      /* 3D */
      var segmentLength3D = Matrix3.Cartesian3.distance(startTop, endTop);
      var encodedStart = EncodedCartesian3.EncodedCartesian3.fromCartesian(startBottom, encodeScratch);
      var forwardOffset = Matrix3.Cartesian3.subtract(endBottom, startBottom, offsetScratch);
      var forward = Matrix3.Cartesian3.normalize(forwardOffset, rightScratch);
      var startUp = Matrix3.Cartesian3.subtract(startTop, startBottom, startUpScratch);
      startUp = Matrix3.Cartesian3.normalize(startUp, startUp);
      var rightNormal = Matrix3.Cartesian3.cross(forward, startUp, rightScratch);
      rightNormal = Matrix3.Cartesian3.normalize(rightNormal, rightNormal);
      var startPlaneNormal = Matrix3.Cartesian3.cross(startUp, startGeometryNormal, startPlaneNormalScratch);
      startPlaneNormal = Matrix3.Cartesian3.normalize(startPlaneNormal, startPlaneNormal);
      var endUp = Matrix3.Cartesian3.subtract(endTop, endBottom, endUpScratch);
      endUp = Matrix3.Cartesian3.normalize(endUp, endUp);
      var endPlaneNormal = Matrix3.Cartesian3.cross(endGeometryNormal, endUp, endPlaneNormalScratch);
      ...
      var encodedStart2D = void 0;
      var forwardOffset2D = void 0;
      var right2D = void 0;
      ...
      for (j = 0; j < 8; j++) {
        ...
        Matrix3.Cartesian3.pack(endPlaneNormal, endNormalAndTextureCoordinateNormalizationX, vec4Index);
        endNormalAndTextureCoordinateNormalizationX[wIndex] = texcoordNormalization3DX * rightPlaneSide;
        Matrix3.Cartesian3.pack(rightNormal, rightNormalAndTextureCoordinateNormalizationY, vec4Index);
      ...
      var getHeightsRectangle = Matrix2.Rectangle.fromCartographicArray(getHeightCartographics, getHeightRectangleScratch);
      var minMaxHeights = ApproximateTerrainHeights$1.getMinimumMaximumHeights(getHeightsRectangle, ellipsoid);
      ...
      var normalNudge = Matrix3.Cartesian3.multiplyByScalar(rightNormal, Math$1.CesiumMath.EPSILON5, normalNudgeScratch);
      ...
      normalNudge = Matrix3.Cartesian3.multiplyByScalar(rightNormal, -2.0 * Math$1.CesiumMath.EPSILON5, normalNudgeScratch);
      Matrix3.Cartesian3.add(adjustHeightStartBottom, normalNudge, adjustHeightStartBottom);
      Matrix3.Cartesian3.add(adjustHeightEndBottom, normalNudge, adjustHeightEndBottom);
      Matrix3.Cartesian3.add(adjustHeightStartTop, normalNudge, adjustHeightStartTop);
      Matrix3.Cartesian3.add(adjustHeightEndTop, normalNudge, adjustHeightEndTop);
    ...
    Transforms.BoundingSphere.fromVertices(bottomPositionsArray, Matrix3.Cartesian3.ZERO, 3, boundingSpheres[0]);
    Transforms.BoundingSphere.fromVertices(topPositionsArray, Matrix3.Cartesian3.ZERO, 3, boundingSpheres[1]);
