define(['./defaultValue-0a909f67', './Matrix3-315394f6', './Matrix2-13178034', './Transforms-40229881', './Check-666ab1a0', './ComponentDatatype-f7b11d02', './GeometryAttribute-7d6f1732', './GeometryAttributes-f06a2792', './GeometryInstance-451dc1cd', './GeometryOffsetAttribute-04332ce7', './GeometryPipeline-ce4339ed', './IndexDatatype-a55ceaa1', './Math-2dbd6b93', './PolygonPipeline-844aab0a', './RectangleGeometryLibrary-bdba697e', './VertexFormat-6b480673', './RuntimeError-06c93819', './combine-ca22a614', './WebGLConstants-a8cc3e8c', './AttributeCompression-b646d393', './EncodedCartesian3-81f70735', './IntersectionTests-f6e6bd8a', './Plane-900aa728', './EllipsoidRhumbLine-19756602'], function (defaultValue, Matrix3, Matrix2, Transforms, Check, ComponentDatatype, GeometryAttribute, GeometryAttributes, GeometryInstance, GeometryOffsetAttribute, GeometryPipeline, IndexDatatype, Math$1, PolygonPipeline, RectangleGeometryLibrary, VertexFormat, RuntimeError, combine, WebGLConstants, AttributeCompression, EncodedCartesian3, IntersectionTests, Plane, EllipsoidRhumbLine) {
  ...
  function calculateAttributes(positions, vertexFormat, ellipsoid, tangentRotationMatrix) {
    ...
    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
      for (var i = 0; i < length; i += 3) {
        var p = Matrix3.Cartesian3.fromArray(positions, i, positionScratch);
  ...
  var v1Scratch = new Matrix3.Cartesian3();
  var v2Scratch = new Matrix3.Cartesian3();
  function calculateAttributesWall(positions, vertexFormat, ellipsoid) {
    ...
    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
      for (var i = 0; i < length; i += 6) {
        var p = Matrix3.Cartesian3.fromArray(positions, i, positionScratch);
        var p1 = Matrix3.Cartesian3.fromArray(positions, (i + 6) % length, v1Scratch);
        if (recomputeNormal) {
          var p2 = Matrix3.Cartesian3.fromArray(positions, (i + 3) % length, v2Scratch);
  ...
  function constructExtrudedRectangle(rectangleGeometry, computedOptions) {
    ...
    var offsetValue;
    var hasOffsets = defaultValue.defined(offsetAttributeValue);
    if (hasOffsets) {
      var size = length / 3 * 2;
    ...
    for (i = 0; i < length - 1; i += 2) {
      ...
      var p1 = Matrix3.Cartesian3.fromArray(wallPositions, upperLeft * 3, v1Scratch);
      var p2 = Matrix3.Cartesian3.fromArray(wallPositions, upperRight * 3, v2Scratch);
    ...
    geo = GeometryPipeline.GeometryPipeline.combineInstances([new GeometryInstance.GeometryInstance({
  ...
  var scratchRectanglePoints = [new Matrix3.Cartesian3(), new Matrix3.Cartesian3(), new Matrix3.Cartesian3(), new Matrix3.Cartesian3()];
  var nwScratch = new Matrix3.Cartographic();
  var stNwScratch = new Matrix3.Cartographic();
  ...
  var tangentRotationMatrixScratch = new Matrix3.Matrix3();
  ...
  RectangleGeometry.createGeometry = function (rectangleGeometry) {
    if (Math$1.CesiumMath.equalsEpsilon(rectangleGeometry._rectangle.north, rectangleGeometry._rectangle.south, Math$1.CesiumMath.EPSILON10) || Math$1.CesiumMath.equalsEpsilon(rectangleGeometry._rectangle.east, rectangleGeometry._rectangle.west, Math$1.CesiumMath.EPSILON10)) {
      return undefined;
    }
    ...
    var extrude = !Math$1.CesiumMath.equalsEpsilon(surfaceHeight, extrudedHeight, 0, Math$1.CesiumMath.EPSILON2);
  ...
  var rectangleCenterScratch = new Matrix3.Cartographic();
  function textureCoordinateRotationPoints(rectangleGeometry) {
    if (rectangleGeometry._stRotation === 0.0) {
      return [0, 0, 0, 1, 1, 0];
    }
    var rectangle = Matrix2.Rectangle.clone(rectangleGeometry._rectangle, unrotatedTextureRectangleScratch);
    var granularity = rectangleGeometry._granularity;
    var ellipsoid = rectangleGeometry._ellipsoid;
    ...
    var toDesiredInComputed = Matrix2.Matrix2.fromRotation(rectangleGeometry._stRotation, rotation2DScratch);
    var boundingRectangleCenter = Matrix2.Rectangle.center(boundingRectangle, rectangleCenterScratch);
    for (var i = 0; i < 3; ++i) {
