define(['exports', './Matrix3-315394f6', './defaultValue-0a909f67', './Check-666ab1a0', './Transforms-40229881', './Math-2dbd6b93'], function (exports, Matrix3, defaultValue, Check, Transforms, Math$1) {
  'use strict';
  ...
  function original(a3, a2, a1, a0) {
    ...
    var cubicRoots = CubicRealPolynomial$1.computeRealRoots(1.0, 2.0 * p, p * p - 4.0 * r, -q * q);
    if (cubicRoots.length > 0) {
      ...
      if (Math.abs(hSquared) < Math$1.CesiumMath.EPSILON14) {
        // y^4 + p y^2 + r = 0.
        var roots = QuadraticRealPolynomial$1.computeRealRoots(1.0, p, r);
        if (roots.length === 2) {
          var root0 = roots[0];
          var root1 = roots[1];
          var y;
        ...
        var roots1 = QuadraticRealPolynomial$1.computeRealRoots(1.0, h, m);
        var roots2 = QuadraticRealPolynomial$1.computeRealRoots(1.0, -h, n);
        if (roots1.length !== 0) {
          roots1[0] += temp;
          roots1[1] += temp;
          if (roots2.length !== 0) {
  ...
  function neumark(a3, a2, a1, a0) {
    ...
    var cubicRoots = CubicRealPolynomial$1.computeRealRoots(1.0, p, q, r);
    if (cubicRoots.length > 0) {
      ...
      var roots1 = QuadraticRealPolynomial$1.computeRealRoots(1.0, G, H);
      var roots2 = QuadraticRealPolynomial$1.computeRealRoots(1.0, g, h);
      if (roots1.length !== 0) {
        if (roots2.length !== 0) {
          if (roots1[1] <= roots2[0]) {
            return [roots1[0], roots1[1], roots2[0], roots2[1]];
  ...
  IntersectionTests.rayPlane = function (ray, plane, result) {
    ...
    var denominator = Matrix3.Cartesian3.dot(normal, direction);
    if (Math.abs(denominator) < Math$1.CesiumMath.EPSILON15) {
      // Ray is parallel to plane.  The ray may be in the polygon's plane.
      return undefined;
    }
    var t = (-plane.distance - Matrix3.Cartesian3.dot(normal, origin)) / denominator;
    if (t < 0) {
      return undefined;
    }
  ...
  var scratchEdge0 = new Matrix3.Cartesian3();
  var scratchEdge1 = new Matrix3.Cartesian3();
  var scratchPVec = new Matrix3.Cartesian3();
  var scratchTVec = new Matrix3.Cartesian3();
  var scratchQVec = new Matrix3.Cartesian3();
  ...
  IntersectionTests.rayTriangleParametric = function (ray, p0, p1, p2, cullBackFaces) {
    ...
    var origin = ray.origin;
    var direction = ray.direction;
    var edge0 = Matrix3.Cartesian3.subtract(p1, p0, scratchEdge0);
    var edge1 = Matrix3.Cartesian3.subtract(p2, p0, scratchEdge1);
    var p = Matrix3.Cartesian3.cross(direction, edge1, scratchPVec);
    var det = Matrix3.Cartesian3.dot(edge0, p);
    var tvec;
    var q;
    var u;
    var v;
  ...
  function raySphere(ray, sphere, result) {
    ...
    var diff = Matrix3.Cartesian3.subtract(origin, center, scratchPVec);
    var a = Matrix3.Cartesian3.dot(direction, direction);
    var b = 2.0 * Matrix3.Cartesian3.dot(direction, diff);
    var c = Matrix3.Cartesian3.magnitudeSquared(diff) - radiusSquared;
  ...
  IntersectionTests.lineSegmentSphere = function (p0, p1, sphere, result) {
    ...
    var direction = Matrix3.Cartesian3.subtract(p1, p0, ray.direction);
    var maxT = Matrix3.Cartesian3.magnitude(direction);
    Matrix3.Cartesian3.normalize(direction, direction);
    result = raySphere(ray, sphere, result);
  ...
  var scratchQ = new Matrix3.Cartesian3();
  var scratchW = new Matrix3.Cartesian3();
  ...
  IntersectionTests.rayEllipsoid = function (ray, ellipsoid) {
    ...
    var q = Matrix3.Cartesian3.multiplyComponents(inverseRadii, ray.origin, scratchQ);
    var w = Matrix3.Cartesian3.multiplyComponents(inverseRadii, ray.direction, scratchW);
    var q2 = Matrix3.Cartesian3.magnitudeSquared(q);
    var qw = Matrix3.Cartesian3.dot(q, w);
  ...
  function quadraticVectorExpression(A, b, c, x, w) {
    var xSquared = x * x;
    var wSquared = w * w;
    var l2 = (A[Matrix3.Matrix3.COLUMN1ROW1] - A[Matrix3.Matrix3.COLUMN2ROW2]) * wSquared;
    var l1 = w * (x * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN1ROW0], A[Matrix3.Matrix3.COLUMN0ROW1], Math$1.CesiumMath.EPSILON15) + b.y);
    var l0 = A[Matrix3.Matrix3.COLUMN0ROW0] * xSquared + A[Matrix3.Matrix3.COLUMN2ROW2] * wSquared + x * b.x + c;
    var r1 = wSquared * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN2ROW1], A[Matrix3.Matrix3.COLUMN1ROW2], Math$1.CesiumMath.EPSILON15);
    var r0 = w * (x * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN2ROW0], A[Matrix3.Matrix3.COLUMN0ROW2]) + b.z);
    var cosines;
    var solutions = [];
    if (r0 === 0.0 && r1 === 0.0) {
  ...
  var firstAxisScratch = new Matrix3.Cartesian3();
  var secondAxisScratch = new Matrix3.Cartesian3();
  var thirdAxisScratch = new Matrix3.Cartesian3();
  var referenceScratch = new Matrix3.Cartesian3();
  var bCart = new Matrix3.Cartesian3();
  var bScratch = new Matrix3.Matrix3();
  var btScratch = new Matrix3.Matrix3();
  var diScratch = new Matrix3.Matrix3();
  var dScratch = new Matrix3.Matrix3();
  var cScratch = new Matrix3.Matrix3();
  var tempMatrix = new Matrix3.Matrix3();
  var aScratch = new Matrix3.Matrix3();
  var sScratch = new Matrix3.Cartesian3();
  var closestScratch = new Matrix3.Cartesian3();
  var surfPointScratch = new Matrix3.Cartographic();
  ...
  IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
    ...
    var firstAxis = Matrix3.Cartesian3.normalize(f, f);
    var reference = Matrix3.Cartesian3.mostOrthogonalAxis(f, referenceScratch);
    var secondAxis = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(reference, firstAxis, secondAxisScratch), secondAxisScratch);
    var thirdAxis = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(firstAxis, secondAxis, thirdAxisScratch), thirdAxisScratch);
    var B = bScratch;
    B[0] = firstAxis.x;
    B[1] = firstAxis.y;
    B[2] = firstAxis.z;
    ...
    B[7] = thirdAxis.y;
    B[8] = thirdAxis.z;
    var B_T = Matrix3.Matrix3.transpose(B, btScratch);

    // Get the scaling matrix and its inverse.
    var D_I = Matrix3.Matrix3.fromScale(ellipsoid.radii, diScratch);
    var D = Matrix3.Matrix3.fromScale(ellipsoid.oneOverRadii, dScratch);
    var C = cScratch;
    C[0] = 0.0;
    C[1] = -direction.z;
    C[2] = direction.y;
    ...
    C[7] = direction.x;
    C[8] = 0.0;
    var temp = Matrix3.Matrix3.multiply(Matrix3.Matrix3.multiply(B_T, D, tempMatrix), C, tempMatrix);
    var A = Matrix3.Matrix3.multiply(Matrix3.Matrix3.multiply(temp, D_I, aScratch), B, aScratch);
    var b = Matrix3.Matrix3.multiplyByVector(temp, position, bCart);

    // Solve for the solutions to the expression in standard form:
    var solutions = quadraticVectorExpression(A, Matrix3.Cartesian3.negate(b, firstAxisScratch), 0.0, 0.0, 1.0);
    var s;
    var altitude;
    var length = solutions.length;
    if (length > 0) {
      var closest = Matrix3.Cartesian3.clone(Matrix3.Cartesian3.ZERO, closestScratch);
      var maximumValue = Number.NEGATIVE_INFINITY;
      for (var i = 0; i < length; ++i) {
        s = Matrix3.Matrix3.multiplyByVector(D_I, Matrix3.Matrix3.multiplyByVector(B, solutions[i], sScratch), sScratch);
        var v = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.subtract(s, position, referenceScratch), referenceScratch);
        var dotProduct = Matrix3.Cartesian3.dot(v, direction);
      ...
      altitude = Matrix3.Cartesian3.magnitude(Matrix3.Cartesian3.subtract(closest, position, referenceScratch)) * Math.sqrt(1.0 - maximumValue * maximumValue);
  ...
  var lineSegmentPlaneDifference = new Matrix3.Cartesian3();
  ...
  IntersectionTests.lineSegmentPlane = function (endPoint0, endPoint1, plane, result) {
    ...
    var difference = Matrix3.Cartesian3.subtract(endPoint1, endPoint0, lineSegmentPlaneDifference);
    var normal = plane.normal;
    var nDotDiff = Matrix3.Cartesian3.dot(normal, difference);

    // check if the segment and plane are parallel
    if (Math.abs(nDotDiff) < Math$1.CesiumMath.EPSILON6) {
      return undefined;
    }
    var nDotP0 = Matrix3.Cartesian3.dot(normal, endPoint0);
  ...
  IntersectionTests.trianglePlaneIntersection = function (p0, p1, p2, plane) {
    ...
    var p0Behind = Matrix3.Cartesian3.dot(planeNormal, p0) + planeD < 0.0;
    var p1Behind = Matrix3.Cartesian3.dot(planeNormal, p1) + planeD < 0.0;
    var p2Behind = Matrix3.Cartesian3.dot(planeNormal, p2) + planeD < 0.0;
